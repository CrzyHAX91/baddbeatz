...
class TestAIChat:
    ...
    def test_ai_chat_input_validation(self, client):
        ...
        # Test empty question with auth token should return 400
        # Use a valid token by registering a user
        token = None
        register_response = client.post('/api/register', json={'username': 'testuser123', 'password': 'testpass123'})
    ...
    def test_ai_chat_fallback(self, client):
        ...
        with patch.dict('sys.modules', {'worker_logic': None, 'gemini_logic': None, 'huggingface_logic': None}):
            response = client.post('/api/ask', json={'question': 'What music do you play?'})
            assert response.status_code == 200
            data = response.get_json()
    ...
    def test_ai_chat_with_worker_logic(self, mock_ai_ask, client):
