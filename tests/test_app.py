...
def test_app_serves_index(tmp_path):
    ...
    proc = subprocess.Popen(["python3", "app.py"], env=env, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    try:
        time.sleep(2)
...
def test_app_tracks_endpoint(tmp_path):
    ...
    try:
        ...
        res = requests.post(f"http://localhost:{port}/api/tracks", json=payload)
